...
# v1 long-only strategy validation: includes config + demo_exchange-filtered trades for journal proof.
...
curl -sS "$BASE_URL/api/config" | tee "$DIR/config.json" >/dev/null
...
curl -sS "$BASE_URL/api/analytics/trades?source=demo_exchange&epoch=crypto_v1_demo&limit=200" | tee "$DIR/analytics_trades_demo_exchange.json" >/dev/null

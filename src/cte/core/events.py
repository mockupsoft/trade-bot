...
class SubScoreBreakdown(BaseModel):
    """Individual sub-score detail for auditability."""
...
class ScoredSignalEvent(BaseEvent):
    """Signal event produced by the weighted scoring signal engine.

    Every field is designed for auditability: you can reconstruct
    exactly why this signal was generated by examining the sub_scores,
    weights, gates, and reason.
    """
    ...
    context_multiplier: float = Field(ge=0.0, le=1.0)
    tier: SignalTier

    # Sub-score breakdown
    sub_scores: dict[str, float] = Field(default_factory=dict)
    weights: dict[str, float] = Field(default_factory=dict)
    sub_score_details: dict[str, SubScoreBreakdown] = Field(default_factory=dict)

    # Gate results
    gates_passed: bool = True
    gate_results: list[GateCheckResult] = Field(default_factory=list)

    # Reason (human + machine readable)
    reason: SignalReason
...
STREAM_KEYS = {
    ...
    "signal_scored": "cte:signal:scored",

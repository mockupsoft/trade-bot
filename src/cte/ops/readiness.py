...
class DashboardPaperToTestnetMetrics:
    testnet_keys: bool = False
    market_connected: bool = False
    v1_safe_not_live: bool = False
    paper_trades: int = 0
    paper_days: int = 0
    crash_free_days: int = 0
    all_tests_pass: bool = False
...
def build_paper_to_demo_checklist(
    ...
    return [
        ReadinessGate(
            ...
            value=str(metrics.paper_days), threshold="7",
        ),
        ReadinessGate(
            ...
            value=str(metrics.paper_trades), threshold="50",
        ),
        ReadinessGate(
            ...
            value=str(metrics.crash_free_days), threshold="7",
        ),
        ReadinessGate(
        ...
        ReadinessGate(
            ...
            status=GateStatus.PASS if metrics.state_machine_violations == 0 else GateStatus.FAIL,
            value=str(metrics.state_machine_violations), threshold="0",
        ),
        ReadinessGate(
            name="api_keys", category="infrastructure",
            description="Testnet API keys configured",
            status=GateStatus.PASS if metrics.api_keys_configured else GateStatus.FAIL,
        ),
    ]


def build_demo_to_live_checklist(
    metrics: DemoToLiveMetrics,
) -> list[ReadinessGate]:
    return [
        ReadinessGate(
            name="demo_duration", category="validation",
            description="Demo trading ran for ≥7 consecutive days",
            status=GateStatus.PASS if metrics.demo_days >= 7 else GateStatus.FAIL,
            value=str(metrics.demo_days), threshold="7",
        ),
        ReadinessGate(
            name="demo_trade_count", category="validation",
            description="50-trade acceptance test passed on testnet",
            status=GateStatus.PASS if metrics.demo_trades >= 50 else GateStatus.FAIL,
            value=str(metrics.demo_trades), threshold="50",
        ),
        ReadinessGate(
            name="reconciliation", category="validation",
            description="100% clean reconciliation for 7 days",
            status=GateStatus.PASS if metrics.reconciliation_clean_rate >= 1.0 else GateStatus.FAIL,
            value=f"{metrics.reconciliation_clean_rate:.0%}", threshold="100%",
        ),
        ReadinessGate(
            name="fill_latency", category="performance",
            description="Fill latency p99 < 5 seconds (measured)",
            status=GateStatus.PASS
            if metrics.fill_latency_p99_ms > 0 and metrics.fill_latency_p99_ms < 5000
            else GateStatus.FAIL,
            value=f"{metrics.fill_latency_p99_ms:.0f}ms", threshold="<5000ms",
        ),
        ReadinessGate(
            name="pnl_parity", category="validation",
            description="Paper-demo PnL within 5% for same signals",
            status=GateStatus.PASS if abs(metrics.paper_demo_pnl_drift_pct) < 5 else GateStatus.FAIL,
            value=f"{metrics.paper_demo_pnl_drift_pct:.1f}%", threshold="±5%",
        ),
        ReadinessGate(
            name="slippage_drift", category="validation",
            description="Slippage drift < 3 bps vs paper model",
            status=GateStatus.PASS if 0 <= metrics.slippage_drift_bps < 3.0 else GateStatus.FAIL,
            value=f"{metrics.slippage_drift_bps:.1f}bps", threshold="3.0bps",
        ),
        ReadinessGate(
            name="emergency_stop", category="ops",
            description="Emergency stop tested and functional",
            status=GateStatus.PASS if metrics.emergency_stop_tested else GateStatus.FAIL,
        ),
        ReadinessGate(
            name="manual_review", category="ops",
            description="Team review and sign-off complete",
            status=GateStatus.PASS if metrics.manual_review_signed else GateStatus.FAIL,
        ),
        ReadinessGate(
            name="capital_limits", category="risk",
            description="Max capital ($100) and position ($50) configured",
            status=GateStatus.PASS if metrics.max_capital_configured else GateStatus.FAIL,
        ),
        ReadinessGate(
            name="monitoring", category="ops",
            description="24/7 monitoring alerts configured",
            status=GateStatus.PASS if metrics.monitoring_alerts_configured else GateStatus.FAIL,
        ),
    ]
